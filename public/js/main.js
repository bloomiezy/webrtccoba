...
const screenShareGrid = document.querySelector('#screenShareGrid');
...
const localVideoElement = document.querySelector('#localVideo-container video');
webrtc.getLocalStream({ audio: true }, { video: true }).then((stream) => {
    localVideoElement.srcObject = stream;
});

document.getElementById('shareScreenBtn').addEventListener('click', () => {
    webrtc.startScreenShare().then((stream) => {
        const socketId = webrtc.myId;
        const videoContainer = document.createElement('div');
        videoContainer.setAttribute('class', 'screen-share-item');
        videoContainer.setAttribute('id', `share-${socketId}`);

        const video = document.createElement('video');
        video.setAttribute('autoplay', true);
        video.setAttribute('muted', true); // set to false if you want audio
        video.setAttribute('playsinline', true);
        video.srcObject = stream;

        const p = document.createElement('p');
        p.textContent = `Screen Share: ${socketId}`;

        videoContainer.append(p);
        videoContainer.append(video);
        screenShareGrid.append(videoContainer);

        // Handle screen share stop event
        stream.getVideoTracks()[0].addEventListener('ended', () => {
            document.getElementById(`share-${socketId}`).remove();
        });
    }).catch((error) => {
