...
class Webrtc extends EventTarget {
    ...
    kickUser(socketId) {
        ...
        // Tambahkan ini di dalam class Webrtc di `webrtc.js`
    ...
    startScreenShare() {
      return navigator.mediaDevices.getDisplayMedia({ video: true })
          .then((stream) => {
              this.log('Started screen share.');
              const videoTrack = stream.getVideoTracks()[0];

              // Replace the video track in the local stream
              this._localStream.getVideoTracks()[0].stop();
              this._localStream.removeTrack(this._localStream.getVideoTracks()[0]);
              ...
              videoTrack.addEventListener('ended', () => {
                  ...
                  this.getLocalStream({ audio: true }, { video: true }).then((localStream) => {
                      const localVideoTrack = localStream.getVideoTracks()[0];
                      this._localStream.addTrack(localVideoTrack);
                      Object.keys(this.pcs).forEach(socketId => {
                          this.pcs[socketId].getSenders().forEach(sender => {
                              if (sender.track.kind === 'video') {
          ...
          .catch((error) => {
              this.error('Error starting screen share: ', error);
              this._emit('error', { error: new Error('Error starting screen share: ' + error.message) });
          });
  }
